...
export default function QuizLecon2({ onResult }) {
  ...
  return (
    <div className="mt-8 p-4 rounded-xl bg-gray-50 border border-gray-200">
      <div className="font-bold mb-2">Quiz : testez vos connaissances</div>
      <div className="mb-2">{questions[step].question}</div>
      <div className="space-y-2">
        {questions[step].options.map((opt, idx) => (
          <button
            key={idx}
            className={`block w-full text-left px-4 py-2 rounded-lg border transition focus:outline-none ${selected === idx ? "bg-green-100 border-green-400" : "bg-white border-gray-200 hover:bg-gray-100"}`}
