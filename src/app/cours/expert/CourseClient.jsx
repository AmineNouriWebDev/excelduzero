    ...
    return (
      <div className="bg-gray-50 min-h-screen flex flex-col text-gray-800">
        ...
        <div className="flex-1 flex items-center justify-center">
          <span className="text-purple-700 text-xl font-bold animate-pulse">Vérification de l'accès...</span>
        </div>
        <Footer />
      </div>
    );
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          <div className="bg-white rounded-2xl shadow-xl p-10 max-w-lg border border-gray-100">
            ...
            <p className="text-gray-600 mb-8 text-lg">
              Le niveau Expert est réservé aux membres Premium. Vous devez souscrire à notre offre pour débloquer ces leçons exclusives.
            </p>
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col text-gray-800">
      ...
      <div className="md:hidden w-full px-4 pt-4 pb-2 flex items-center z-40 sticky top-16 bg-gray-50" style={{boxShadow: '0 2px 8px -4px rgba(0,0,0,0.04)'}}>
        <button
          className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-xl font-bold shadow hover:bg-purple-700 transition text-base"
      ...
      <main className="flex-1 pt-24 pb-16 w-full flex gap-0 md:gap-6 max-w-none px-0 sm:px-0 md:px-0 mx-0 items-start">
        ...
        >
          ...
          >
            <div className="w-2 h-12 bg-gray-300 rounded-full opacity-70 group-hover:opacity-100 transition-opacity absolute top-1/2 -translate-y-1/2 flex items-center justify-center">
              <svg className="w-4 h-4 text-gray-400 mx-auto group-hover:text-purple-600 transition-colors" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6h4M10 12h4M10 18h4"/></svg>
            </div>
          </div>
          {sidebarOpen ? (
            <>
              <h2 className="text-lg font-bold text-purple-700 mb-4 text-center">Sommaire</h2>
              <ol className="space-y-1">
                {LEÇONS.map((titre, i) => (
                  <li key={i}>
                    <button
                      className={`w-full flex items-center text-left px-3 py-2 rounded-lg transition font-medium ${active === i ? "bg-purple-100 text-purple-800 font-bold" : "hover:bg-gray-100 text-gray-700"}`}
                      onClick={() => {
                        setActive(i);
            ...
            <ol className="space-y-1 flex flex-col items-center">
              {LEÇONS.map((_, i) => (
                <li key={i}>
                  <button
                    className={`w-8 h-8 flex items-center justify-center rounded-full transition font-bold text-sm ${active === i ? "bg-purple-100 text-purple-800" : "hover:bg-gray-100 text-gray-700"}`}
                    onClick={() => {
                      setActive(i);
                      router.replace(`?lecon=${i+1}`, { scroll: false });
                    }}
        ...
        <section className="flex-1 bg-white md:rounded-2xl rounded-none shadow px-0 py-3 sm:p-6 min-h-[400px] md:mr-2 overflow-x-auto max-w-full">
          ...
          <h2 className="text-2xl font-bold text-purple-700 mb-4 break-words">{LEÇONS[active]}</h2>
